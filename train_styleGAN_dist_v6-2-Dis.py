...
def train(opt):
    ...
    if not opt.zAlone:
        ...
        if opt.cEncode == 'mlp':
            ...
            cEncoder = ContentTensorEncoderV6(n_downsample=4, n_res=0, input_dim=1, dim=32, norm=opt.contentNorm, activ='lrelu').to(device)
            c_code_size = 0 #for 5-1/v6 architecture
        # styleModel = ResNet_StyleExtractor_WBN(opt.input_channel, opt.latent).to(device)
        styleModel = StyleTensorEncoderV6(n_downsample=4, n_res=4, input_dim=opt.input_channel, dim=32, norm=opt.styleNorm, activ='lrelu', numLatents=10).to(device)
