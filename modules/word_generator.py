...
class WordGenerator(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, res_norm='adain', activ='relu', pad_type='zero'):
        ...
        self.res_model += [ResBlocks(n_res, dim, res_norm, activ, pad_type=pad_type)]
        # upsampling blocks
        
        for i in range(len(n_upsample)):
            self.up_model += [nn.Upsample(scale_factor=n_upsample[i]),
                           Conv2dBlock(dim, dim // 2, 3, 1, 1, norm='ln', activation=activ, pad_type=pad_type)]
            dim //= 2
        # use reflection padding in the last conv layer
        self.up_model += [Conv2dBlock(dim, output_dim, 3, 1, 1, norm='none', activation='tanh', pad_type=pad_type)]
        self.res_model = nn.Sequential(*self.res_model)
        self.up_model = nn.Sequential(*self.up_model)
