...
def train(opt):
    ...
    while(True):
        # train part
        # pdb.set_trace()
        for i, (image_tensors_all, labels_1_all) in enumerate(train_loader):
            ...
            if 'CTC' in opt.Prediction:
                ...
                # ocrCost = 0.5*(ocrCost_1+ocrCost_2)
                print('OCR shape::::::',preds_1.shape,target_1.shape,preds_2.shape,target_2.shape)
            if not opt.ocrFixed:
                #training OCR
                ocrModel.zero_grad()
            ...
            recCost.backward(retain_graph=True)
