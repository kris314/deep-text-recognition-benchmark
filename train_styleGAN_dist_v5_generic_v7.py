...
from model_word import GeneratorM2V4_4 as styleGANGen 
...
def train(opt):
    ...
    loss_avg_ocr_mask = Averager()
    loss_avg_ocr_invmask = Averager()
    ...
    while(True):
        ...
        if not opt.testFlag:
            ...
            fake_img,_,_ = genModel(style, z_c_code, input_is_latent=opt.input_latent)
            ...
            fake_img,_, fake_img_bin = genModel(style, z_c_code, input_is_latent=opt.input_latent)
            ...
            if opt.zAlone:
                ...
                if 'CTC' in opt.Prediction:
                    ...
                    preds_recon_bin = ocrModel((fake_img_bin-0.5)/0.5, text_z_c, is_train=False, inAct = opt.taskActivation)
                    preds_size_bin = torch.IntTensor([preds_recon_bin.size(1)] * opt.batch_size)
                    preds_recon_softmax_bin = preds_recon_bin.log_softmax(2).permute(1, 0, 2)
                    ocrCost_mask = ocrCriterion(preds_recon_softmax_bin, text_z_c, preds_size_bin, length_z_c)

                    preds_recon_invbin = ocrModel((0.5-fake_img_bin)/0.5, text_z_c, is_train=False, inAct = opt.taskActivation)
                    preds_size_invbin = torch.IntTensor([preds_recon_invbin.size(1)] * opt.batch_size)
                    preds_recon_softmax_invbin = preds_recon_invbin.log_softmax(2).permute(1, 0, 2)
                    ocrCost_invmask = ocrCriterion(preds_recon_softmax_invbin, text_z_c, preds_size_invbin, length_z_c)
                    ...
                    preds_recon_bin = ocrModel((fake_img_bin-0.5)/0.5, text_z_c[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                    target = text_z_c[:, 1:]  # without [GO] Symbol
                    ocrCost_mask = ocrCriterion(preds_recon_bin.view(-1, preds_recon_bin.shape[-1]), target.contiguous().view(-1))

                    preds_recon_invbin = ocrModel((0.5-fake_img_bin)/0.5, text_z_c[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                    target = text_z_c[:, 1:]  # without [GO] Symbol
                    ocrCost_invmask = ocrCriterion(preds_recon_invbin.view(-1, preds_recon_invbin.shape[-1]), target.contiguous().view(-1))
                ...
                if opt.reconWeight>0.0:
                    fake_gt_img, _, fake_gt_img_bin = genModel(style, z_gt_code, input_is_latent=opt.input_latent)
                    imgReconCost = 0.5*(reconCriterion(fake_gt_img*torch.round(fake_gt_img_bin), image_input_tensors*fake_gt_img_bin) + reconCriterion(fake_gt_img*(1-fake_gt_img_bin), image_input_tensors*(1-fake_gt_img_bin)))
                else:
                    imgReconCost = torch.tensor(0.0)
                ...
                if opt.cycleReconWeight > 0.0:
                    ...
                    fake_recon_img, _, fake_recon_img_bin = genModel(style_fake, z_gt_code, input_is_latent=opt.input_latent)
                    cycleReconCost = 0.5*(reconCriterion(fake_recon_img*torch.round(fake_recon_img_bin), image_input_tensors*fake_recon_img_bin) + reconCriterion(fake_recon_img*(1-fake_recon_img_bin), image_input_tensors*(1-fake_recon_img_bin)))
                else:
                ...
                if opt.vggStyWeight>0.0:
                    vggPerCost , vggStyCost = vggModel(fake_img, image_input_tensors, inAct=opt.taskActivation)
                else:
                    vggPerCost = torch.tensor(0.0)
                    vggStyCost = torch.tensor(0.0)
            ...
            gen_enc_cost = disGenCost + opt.ocrWeight * ocrCost + opt.ocrMaskWeight * ocrCost_mask + opt.ocrInvMaskWeight * ocrCost_invmask \
                            + opt.gamma_g * uCost + opt.reconWeight * imgReconCost +  opt.vggPerWeight * vggPerCost \
            ...
            loss_avg_ocr_mask.add(opt.ocrMaskWeight * ocrCost_mask)
            loss_avg_ocr_invmask.add(opt.ocrInvMaskWeight * ocrCost_invmask)
            ...
            if not opt.zAlone:
                if get_rank() == 0 and ((iteration + 1) % opt.valInterval == 0 or iteration == 0):
                    ...
                    if 'CTC' in opt.Prediction:
                        ...
                        _, preds_index = preds_recon_bin.max(2)
                        preds_str_fake_img_bin = converter.decode(preds_index.data, preds_size.data)

                        _, preds_index = preds_recon_invbin.max(2)
                        preds_str_fake_img_invbin = converter.decode(preds_index.data, preds_size.data)
                        ...
                        _, preds_index = preds_recon_bin.max(2)
                        preds_str_fake_img_bin = converter.decode(preds_index, length_for_pred)
                        for idx, pred in enumerate(preds_str_fake_img_bin):
                            pred_EOS = pred.find('[s]')
                            preds_str_fake_img_bin[idx] = pred[:pred_EOS]  # prune after "end of sentence" token ([s])
                        
                        _, preds_index = preds_recon_invbin.max(2)
                        preds_str_fake_img_invbin = converter.decode(preds_index, length_for_pred)
                        for idx, pred in enumerate(preds_str_fake_img_invbin):
                            pred_EOS = pred.find('[s]')
                            preds_str_fake_img_invbin[idx] = pred[:pred_EOS]  # prune after "end of sentence" token ([s])
                    ...
                    if opt.reconWeight>0.0:
                        if 'CTC' in opt.Prediction:
                            ...
                            preds = ocrModel((fake_gt_img_bin-0.5)/0.5, text_gt, is_train=False, inAct = opt.taskActivation)
                            preds_size = torch.IntTensor([preds.size(1)] * opt.batch_size)
                            _, preds_index = preds.max(2)
                            preds_str_fake_gt_img_bin = converter.decode(preds_index.data, preds_size.data)

                            preds = ocrModel((0.5-fake_gt_img_bin)/0.5, text_gt, is_train=False, inAct = opt.taskActivation)
                            preds_size = torch.IntTensor([preds.size(1)] * opt.batch_size)
                            _, preds_index = preds.max(2)
                            preds_str_fake_gt_img_invbin = converter.decode(preds_index.data, preds_size.data)
                        else:
                            ...
                            preds = ocrModel((fake_gt_img_bin-0.5)/0.5, text_gt[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                            _, preds_index = preds.max(2)
                            preds_str_fake_gt_img_bin = converter.decode(preds_index, length_for_pred)
                            for idx, pred in enumerate(preds_str_fake_gt_img_bin):
                                pred_EOS = pred.find('[s]')
                                preds_str_fake_gt_img_bin[idx] = pred[:pred_EOS]  # prune after "end of sentence" token ([s])

                            preds = ocrModel((0.5-fake_gt_img_bin)/0.5, text_gt[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                            _, preds_index = preds.max(2)
                            preds_str_fake_gt_img_invbin = converter.decode(preds_index, length_for_pred)
                            for idx, pred in enumerate(preds_str_fake_gt_img_invbin):
                                pred_EOS = pred.find('[s]')
                                preds_str_fake_gt_img_invbin[idx] = pred[:pred_EOS]  # prune after "end of sentence" token ([s])
                    ...
                    if opt.cycleReconWeight>0.0:
                        if 'CTC' in opt.Prediction:
                            ...
                            preds = ocrModel((fake_recon_img_bin-0.5)/0.5, text_gt, is_train=False, inAct = opt.taskActivation)
                            preds_size = torch.IntTensor([preds.size(1)] * opt.batch_size)
                            _, preds_index = preds.max(2)
                            preds_str_fake_recon_img_bin = converter.decode(preds_index.data, preds_size.data)

                            preds = ocrModel((0.5-fake_recon_img_bin)/0.5, text_gt, is_train=False, inAct = opt.taskActivation)
                            preds_size = torch.IntTensor([preds.size(1)] * opt.batch_size)
                            _, preds_index = preds.max(2)
                            preds_str_fake_recon_img_invbin = converter.decode(preds_index.data, preds_size.data)
                            ...
                            preds = ocrModel((fake_recon_img_bin-0.5)/0.5, text_gt[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                            _, preds_index = preds.max(2)
                            preds_str_fake_recon_img_bin = converter.decode(preds_index, length_for_pred)
                            for idx, pred in enumerate(preds_str_fake_recon_img_bin):
                                pred_EOS = pred.find('[s]')
                                preds_str_fake_recon_img_bin[idx] = pred[:pred_EOS]  # prune after "end of sentence" token ([s])
                            
                            preds = ocrModel((0.5-fake_recon_img_bin)/0.5, text_gt[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                            _, preds_index = preds.max(2)
                            preds_str_fake_recon_img_invbin = converter.decode(preds_index, length_for_pred)
                            for idx, pred in enumerate(preds_str_fake_recon_img_invbin):
                                pred_EOS = pred.find('[s]')
                                preds_str_fake_recon_img_invbin[idx] = pred[:pred_EOS]  # prune after "end of sentence" token ([s])
                    ...
                    for trImgCntr in range(opt.batch_size):
                        try:
                            ...
                            utils.save_image(fake_img_bin[trImgCntr],os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_z_pred_bin_'+labels_z_c[trImgCntr]+'_ocr:'+preds_str_fake_img_bin[trImgCntr]+'_sty:'+labels_gt[trImgCntr]+'.png'),nrow=1,normalize=True,range=(0, 1))
                            utils.save_image(1-fake_img_bin[trImgCntr],os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_z_pred_invbin_'+labels_z_c[trImgCntr]+'_ocr:'+preds_str_fake_img_invbin[trImgCntr]+'_sty:'+labels_gt[trImgCntr]+'.png'),nrow=1,normalize=True,range=(0, 1))
                            if opt.reconWeight>0.0:
                                ...
                                utils.save_image(fake_gt_img_bin[trImgCntr],os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_gt_recon_pred_bin_'+labels_gt[trImgCntr]+'_ocr:'+preds_str_fake_gt_img_bin[trImgCntr]+'.png'),nrow=1,normalize=True,range=(0, 1))
                                utils.save_image(1-fake_gt_img_bin[trImgCntr],os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_gt_recon_pred_invbin_'+labels_gt[trImgCntr]+'_ocr:'+preds_str_fake_gt_img_invbin[trImgCntr]+'.png'),nrow=1,normalize=True,range=(0, 1))
                            if opt.cycleReconWeight>0.0:
                                ...
                                utils.save_image(fake_recon_img_bin[trImgCntr],os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_gt_cycle_pred_bin_'+labels_gt[trImgCntr]+'_ocr:'+preds_str_fake_recon_img_bin[trImgCntr]+'.png'),nrow=1,normalize=True,range=(0, 1))
                                utils.save_image(1-fake_recon_img_bin[trImgCntr],os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_gt_cycle_pred_invbin_'+labels_gt[trImgCntr]+'_ocr:'+preds_str_fake_recon_img_invbin[trImgCntr]+'.png'),nrow=1,normalize=True,range=(0, 1))
                        except:
            ...
            if g_regularize:
                ...
                fake_img, latents, fake_img_bin = genModel(style, z_c_code, return_latents=True, input_is_latent=opt.input_latent)
                ...
                if opt.path_batch_shrink:
                    weighted_path_loss += 0 * fake_img[0, 0, 0, 0]  * fake_img_bin[0, 0, 0, 0]
        ...
        if get_rank() == 0 or opt.testFlag:
                ...
                wandb.log(
                    {
                        ...
                        "Train-Mask-OCR-Loss": loss_avg_ocr_mask.val().item(),
                        "Train-InvMask-OCR-Loss": loss_avg_ocr_invmask.val().item(),
            ...
            if (iteration + 1) % opt.valInterval == 0 or opt.testFlag: # To see training progress, we also conduct validation when 'iteration == 0' 
                #validation
                iCntr=0
                ...
                for vCntr, (image_input_tensors, image_output_tensors, labels_gt, labels_z_c, labelSynthImg, synth_z_c) in enumerate(valid_loader):
                    ...
                    if opt.testFlag and opt.realVaData:
                        labels_z_c, synth_z_c = next(text_loader)
                    ...
                    with torch.no_grad():
                        ...
                        fake_img_c1_s1, _, fake_img_c1_s1_bin = g_ema(style, z_gt_code, input_is_latent=opt.input_latent)
                        # print('inside valoidatoin after genModel c1 s1')

                        if not opt.zAlone:
                            fake_img_c2_s1, _, fake_img_c2_s1_bin = g_ema(style, z_c_code, input_is_latent=opt.input_latent)
                        ...
                        if not opt.zAlone:
                            ...
                            if 'CTC' in opt.Prediction:
                                ...
                                preds = ocrModel((fake_img_c1_s1_bin-0.5)/0.5, text_gt, is_train=False, inAct = opt.taskActivation)
                                preds_size = torch.IntTensor([preds.size(1)] * opt.batch_size)
                                _, preds_index = preds.max(2)
                                preds_str_fake_img_c1_s1_bin = converter.decode(preds_index.data, preds_size.data)

                                preds = ocrModel((fake_img_c2_s1_bin-0.5)/0.5, text_z_c, is_train=False, inAct = opt.taskActivation)
                                preds_size = torch.IntTensor([preds.size(1)] * opt.batch_size)
                                _, preds_index = preds.max(2)
                                preds_str_fake_img_c2_s1_bin = converter.decode(preds_index.data, preds_size.data)

                                preds = ocrModel((0.5-fake_img_c1_s1_bin)/0.5, text_gt, is_train=False, inAct = opt.taskActivation)
                                preds_size = torch.IntTensor([preds.size(1)] * opt.batch_size)
                                _, preds_index = preds.max(2)
                                preds_str_fake_img_c1_s1_invbin = converter.decode(preds_index.data, preds_size.data)

                                preds = ocrModel((0.5-fake_img_c2_s1_bin)/0.5, text_z_c, is_train=False, inAct = opt.taskActivation)
                                preds_size = torch.IntTensor([preds.size(1)] * opt.batch_size)
                                _, preds_index = preds.max(2)
                                preds_str_fake_img_c2_s1_invbin = converter.decode(preds_index.data, preds_size.data)
                                ...
                                preds = ocrModel((fake_img_c1_s1_bin-0.5)/0.5, text_gt[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                                _, preds_index = preds.max(2)
                                preds_str_fake_img_c1_s1_bin = converter.decode(preds_index, length_for_pred)
                                for idx, pred in enumerate(preds_str_fake_img_c1_s1_bin):
                                    pred_EOS = pred.find('[s]')
                                    preds_str_fake_img_c1_s1_bin[idx] = pred[:pred_EOS]  # prune after "end of sentence" token ([s])
                                
                                preds = ocrModel((fake_img_c2_s1_bin-0.5)/0.5, text_z_c[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                                _, preds_index = preds.max(2)
                                preds_str_fake_img_c2_s1_bin = converter.decode(preds_index, length_for_pred)
                                for idx, pred in enumerate(preds_str_fake_img_c2_s1_bin):
                                    pred_EOS = pred.find('[s]')
                                    preds_str_fake_img_c2_s1_bin[idx] = pred[:pred_EOS]  # prune after "end of sentence" token ([s])
                                

                                preds = ocrModel((0.5-fake_img_c1_s1_bin)/0.5, text_gt[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                                _, preds_index = preds.max(2)
                                preds_str_fake_img_c1_s1_invbin = converter.decode(preds_index, length_for_pred)
                                for idx, pred in enumerate(preds_str_fake_img_c1_s1_invbin):
                                    pred_EOS = pred.find('[s]')
                                    preds_str_fake_img_c1_s1_invbin[idx] = pred[:pred_EOS]  # prune after "end of sentence" token ([s])
                                
                                preds = ocrModel((0.5-fake_img_c2_s1_bin)/0.5, text_z_c[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                                _, preds_index = preds.max(2)
                                preds_str_fake_img_c2_s1_invbin = converter.decode(preds_index, length_for_pred)
                                for idx, pred in enumerate(preds_str_fake_img_c2_s1_invbin):
                                    pred_EOS = pred.find('[s]')
                                    preds_str_fake_img_c2_s1_invbin[idx] = pred[:pred_EOS]  # prune after "end of sentence" token ([s])
                    ...
                    for trImgCntr in range(opt.batch_size):
                        ...
                        try:
                            ...
                            img_path_c1_s1_bin = os.path.join(str(iCntr)+'_pred_val_c1_s1_bin_'+labels_gt[trImgCntr]+'_ocr:'+preds_str_fake_img_c1_s1_bin[trImgCntr]+'.png')
                            # img_path_c1_s1_invbin = os.path.join(str(iCntr)+'_pred_val_c1_s1_invbin_'+labels_gt[trImgCntr]+'_ocr:'+preds_str_fake_img_c1_s1_invbin[trImgCntr]+'.png')
                            ...
                            img_path_c2_s1_bin = os.path.join(str(iCntr)+'_pred_val_c2_s1_bin_'+labels_z_c[trImgCntr]+'_ocr:'+preds_str_fake_img_c2_s1_bin[trImgCntr]+'.png')
                            # img_path_c2_s1_invbin = os.path.join(str(iCntr)+'_pred_val_c2_s1_invbin_'+labels_z_c[trImgCntr]+'_ocr:'+preds_str_fake_img_c2_s1_invbin[trImgCntr]+'.png')
                            
                            # ims.append([img_path_gt_1, img_path_c1_s1, img_path_c1_s1_bin, img_path_c1_s1_invbin, img_path_gt_2, img_path_c2_s1, img_path_c2_s1_bin, img_path_c2_s1_invbin])
                            ims.append([img_path_gt_1, img_path_c1_s1, img_path_c1_s1_bin, img_path_gt_2, img_path_c2_s1, img_path_c2_s1_bin])

                            content_c1_s1 = 'PSTYLE-1 '+'Text-1:' + labels_gt[trImgCntr]+' OCR:' + preds_str_fake_img_c1_s1[trImgCntr]
                            content_c1_s1_bin = 'PSTYLE-FG-1 '+'Text-1:' + labels_gt[trImgCntr]+' OCR:' + preds_str_fake_img_c1_s1_bin[trImgCntr]
                            # content_c1_s1_invbin = 'PSTYLE-BG-1 '+'Text-1:' + labels_gt[trImgCntr]+' OCR:' + preds_str_fake_img_c1_s1_invbin[trImgCntr]
                            ...
                            content_c2_s1_bin = 'PSTYLE-FG-1 '+'Text-2:' + labels_z_c[trImgCntr]+' OCR:'+preds_str_fake_img_c2_s1_bin[trImgCntr]
                            # content_c2_s1_invbin = 'PSTYLE-BG-1 '+'Text-2:' + labels_z_c[trImgCntr]+' OCR:'+preds_str_fake_img_c2_s1_invbin[trImgCntr]
                            
                            # txts.append([content_gt_1, content_c1_s1, content_c1_s1_bin, content_c1_s1_invbin, content_gt_2, content_c2_s1, content_c2_s1_bin, content_c2_s1_invbin])
                            txts.append([content_gt_1, content_c1_s1, content_c1_s1_bin, content_gt_2, content_c2_s1, content_c2_s1_bin])
                            ...
                            utils.save_image(fake_img_c1_s1_bin[trImgCntr],os.path.join(pathPrefix,str(iCntr)+'_pred_val_c1_s1_bin_'+labels_gt[trImgCntr]+'_ocr:'+preds_str_fake_img_c1_s1_bin[trImgCntr]+'.png'),nrow=1,normalize=True,range=(0, 1))
                            # utils.save_image(1-fake_img_c1_s1_bin[trImgCntr],os.path.join(pathPrefix,str(iCntr)+'_pred_val_c1_s1_invbin_'+labels_gt[trImgCntr]+'_ocr:'+preds_str_fake_img_c1_s1_invbin[trImgCntr]+'.png'),nrow=1,normalize=True,range=(0, 1))
                            ...
                            if not opt.zAlone:
                                ...
                                utils.save_image(fake_img_c2_s1_bin[trImgCntr],os.path.join(pathPrefix,str(iCntr)+'_pred_val_c2_s1_bin_'+labels_z_c[trImgCntr]+'_ocr:'+preds_str_fake_img_c2_s1_bin[trImgCntr]+'.png'),nrow=1,normalize=True,range=(0, 1))
                                # utils.save_image(1-fake_img_c2_s1_bin[trImgCntr],os.path.join(pathPrefix,str(iCntr)+'_pred_val_c2_s1_invbin_'+labels_z_c[trImgCntr]+'_ocr:'+preds_str_fake_img_c2_s1_invbin[trImgCntr]+'.png'),nrow=1,normalize=True,range=(0, 1))
                ...
                if not opt.testFlag:
                    ...
                    with open(os.path.join(opt.exp_dir,opt.exp_name,'log_train.txt'), 'a') as log:
                        ...
                        loss_log = f'[{iteration+1}/{opt.num_iter}]  \
                            ...
                            Train Mask OCR loss: {loss_avg_ocr_mask.val():0.5f}, \
                            Train InvMask OCR loss: {loss_avg_ocr_invmask.val():0.5f}, \
                        ...
                        lib.plot.plot(os.path.join(opt.plotDir,'Train-Mask-OCR-Loss'), loss_avg_ocr_mask.val().item())
                        lib.plot.plot(os.path.join(opt.plotDir,'Train-InvMask-OCR-Loss'), loss_avg_ocr_invmask.val().item())
                        ...
                        loss_avg_ocr_mask.reset()
                        loss_avg_ocr_invmask.reset()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ocrMaskWeight', type=float, default=1.0, help='weights for loss')
    parser.add_argument('--ocrInvMaskWeight', type=float, default=1.0, help='weights for loss')
