        ...
        if not opt.testFlag:
            ...
            if opt.zAlone:
                ...
                if opt.cycleReconWeight > 0.0:
                    ...
                    fake_recon_img, _ = genModel(style_fake, z_c_code, input_is_latent=opt.input_latent)
                    cycleReconCost = reconCriterion(fake_recon_img, fake_img)
                else:
                ...
                if opt.vggPerWeight>0.0 or opt.vggStyWeight>0.0 or opt.vggEmbWeight>0.0:
                    vggPerCost , vggStyCost, vggEmbCost  = vggModel(fake_img, image_input_tensors, inAct=opt.taskActivation, normFlag=not(opt.vggNoMean))
                else:
                    vggPerCost = torch.tensor(0.0)
                    vggStyCost = torch.tensor(0.0)
                    vggEmbCost = torch.tensor(0.0)
            ...
            if not opt.zAlone:
                if get_rank() == 0 and ((iteration + 1) % opt.valInterval == 0 or iteration == 0):
                    ...
                    if opt.cycleReconWeight>0.0:
                        if 'CTC' in opt.Prediction:
                            preds = ocrModel(fake_recon_img, text_for_pred, is_train=False, inAct = opt.taskActivation)
                            preds_size = torch.IntTensor([preds.size(1)] * opt.batch_size)
                            _, preds_index = preds.max(2)
                            preds_str_fake_recon_img = converter.decode(preds_index.data, preds_size.data)
                        else:
                            
                            preds = ocrModel(fake_recon_img, text_for_pred[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                            _, preds_index = preds.max(2)
                    ...
                    for trImgCntr in range(opt.batch_size):
                        try:
                            ...
                            utils.save_image(fake_img[trImgCntr],os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_z_recon_pred_'+labels_z_c[trImgCntr]+'_ocr:'+preds_str_fake_img[trImgCntr]+'_sty:'+labels_gt[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))
                            ...
                            if opt.cycleReconWeight>0.0:
                                utils.save_image(fake_recon_img[trImgCntr],os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_z_cycle_pred_'+labels_z_c[trImgCntr]+'_ocr:'+preds_str_fake_recon_img[trImgCntr]+'_sty:'+labels_gt[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))
                        except:
