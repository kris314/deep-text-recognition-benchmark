        ...
        if not opt.testFlag:
            ...
            if opt.zAlone:
                ...
                if 'CTC' in opt.Prediction:
                    ...
                    preds_size = torch.IntTensor([preds_recon.size(1)] * opt.batch_size)
                    preds_recon_softmax = preds_recon.log_softmax(2).permute(1, 0, 2)
                    ocrCost_1 = ocrCriterion(preds_recon_softmax, text_z_c, preds_size, length_z_c)

                    preds_recon_bin = ocrModel(fake_img*fake_img_bin, text_z_c, is_train=False, inAct = opt.taskActivation)
                    ...
                    ocrCost_2 = ocrCriterion(preds_recon_softmax_bin, text_z_c, preds_size_bin, length_z_c)
                    ...
                    ocrCost_1 = ocrCriterion(preds_recon.view(-1, preds_recon.shape[-1]), target.contiguous().view(-1))

                    preds_recon_bin = ocrModel(fake_img*fake_img_bin, text_z_c[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                    target = text_z_c[:, 1:]  # without [GO] Symbol
                    ocrCost_2 = ocrCriterion(preds_recon_bin.view(-1, preds_recon_bin.shape[-1]), target.contiguous().view(-1))

                    # print("Not implemented error")
                    # sys.exit()
                ocrCost = ocrCost_1 + ocrCost_2
                ...
                if get_rank() == 0 and ((iteration + 1) % opt.valInterval == 0 or iteration == 0):
                    ...
                    for trImgCntr in range(opt.batch_size):
                        try:
                            ...
                            utils.save_image(fake_img[trImgCntr]*fake_img_bin[trImgCntr],os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_z_pred_bin_'+labels_z_c[trImgCntr]+'_ocr:'+preds_str_fake_img_bin[trImgCntr]+'_sty:'+labels_gt[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))
                            if opt.reconWeight>0.0:
                                ...
                                utils.save_image(fake_gt_img[trImgCntr]*fake_gt_img_bin[trImgCntr],os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_gt_recon_pred_bin_'+labels_gt[trImgCntr]+'_ocr:'+preds_str_fake_gt_img_bin[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))
                            if opt.cycleReconWeight>0.0:
                                ...
                                utils.save_image(fake_recon_img[trImgCntr]*fake_recon_img_bin[trImgCntr],os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_gt_cycle_pred_bin_'+labels_gt[trImgCntr]+'_ocr:'+preds_str_fake_recon_img_bin[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))
                        except:
                ...
                for vCntr, (image_input_tensors, image_output_tensors, labels_gt, labels_z_c, labelSynthImg, synth_z_c) in enumerate(valid_loader):
                    ...
                    with torch.no_grad():
                        ...
                        if not opt.zAlone:
                            ...
                            if 'CTC' in opt.Prediction:
                                ...
                                preds = ocrModel(fake_img_c1_s1*fake_img_c1_s1_bin, text_gt, is_train=False, inAct = opt.taskActivation)
                                preds_size = torch.IntTensor([preds.size(1)] * opt.batch_size)
                                _, preds_index = preds.max(2)
                                preds_str_fake_img_c1_s1_bin = converter.decode(preds_index.data, preds_size.data)

                                preds = ocrModel(fake_img_c2_s1*fake_img_c2_s1_bin, text_z_c, is_train=False, inAct = opt.taskActivation)
                                preds_size = torch.IntTensor([preds.size(1)] * opt.batch_size)
                                _, preds_index = preds.max(2)
                                ...
                                preds = ocrModel(fake_img_c1_s1*fake_img_c1_s1_bin, text_gt[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                                _, preds_index = preds.max(2)
                                ...
                                preds = ocrModel(fake_img_c2_s1*fake_img_c2_s1_bin, text_z_c[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                                _, preds_index = preds.max(2)
                    ...
                    for trImgCntr in range(opt.batch_size):
                        ...
                        try:
                            ...
                            utils.save_image(fake_img_c1_s1[trImgCntr]*fake_img_c1_s1_bin[trImgCntr],os.path.join(pathPrefix,str(iCntr)+'_pred_val_c1_s1_bin_'+labels_gt[trImgCntr]+'_ocr:'+preds_str_fake_img_c1_s1_bin[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))
                            ...
                            if not opt.zAlone:
                                ...
                                utils.save_image(fake_img_c2_s1[trImgCntr]*fake_img_c2_s1_bin[trImgCntr],os.path.join(pathPrefix,str(iCntr)+'_pred_val_c2_s1_bin_'+labels_z_c[trImgCntr]+'_ocr:'+preds_str_fake_img_c2_s1_bin[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))
