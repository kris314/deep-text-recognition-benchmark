...
from dataset import fontTextDataset, AlignFontCollate
...
def train(opt):
    ...
    train_dataset = fontTextDataset(imgDir=opt.train_img_dir, annFile=opt.train_ann_file, transform=None, numClasses=opt.numClasses)
    ...
    valid_dataset = fontTextDataset(imgDir=opt.train_img_dir, annFile=opt.val_ann_file, transform=None, F2Idx=train_dataset.F2Idx, Idx2F=train_dataset.Idx2F, numClasses=opt.numClasses)
                    ...
                    with torch.no_grad():
                        ...
                        if opt.testFlag:
                            ...
                            for vCntr in range(preds_max.shape[0]):
                                print('Actual=', train_dataset.Idx2F[labels_gt[vCntr]], 'Predicted=',train_dataset.Idx2F[preds_max[vCntr]])
