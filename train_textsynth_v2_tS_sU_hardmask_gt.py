        ...
        if not opt.testFlag:
            ...
            if opt.zAlone:
                ...
                if 'CTC' in opt.Prediction:
                    ...
                    preds_recon_bin = ocrModel((fake_gt_img_bin-0.5)/0.5, text_gt, is_train=False, inAct = opt.taskActivation)
                    ...
                    ocrCost_mask = ocrCriterion(preds_recon_softmax_bin, text_gt, preds_size_bin, length_gt)
                    ...
                    preds_recon_bin = ocrModel((fake_gt_img_bin-0.5)/0.5, text_gt[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                    target = text_gt[:, 1:]  # without [GO] Symbol
                ...
                if opt.cycleReconWeight > 0.0:
                    ...
                    fake_recon_img, _, fake_recon_img_bin = genModel(fake_gt_img, style_fake, z_gt_code, input_is_latent=opt.input_latent)
                ...
                if get_rank() == 0 and ((iteration + 1) % opt.valInterval == 0 or iteration == 0):
                    ...
                    for trImgCntr in range(opt.batch_size):
                        try:
                            ...
                            utils.save_image(fake_sty_img_bin[trImgCntr],os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_z_predBin_'+labels_z_c[trImgCntr]+'_ocr:None_sty:'+labels_gt[trImgCntr]+'.png'),nrow=1,normalize=True,range=(0, 1))
                            if opt.cycleReconWeight>0.0:
            ...
            if g_regularize:
                ...
                fake_gt_img, latents, fake_gt_img_bin = genModel(image_input_tensors, style, z_gt_code, return_latents=True, input_is_latent=opt.input_latent)
                ...
                if opt.path_batch_shrink:
                    weighted_path_loss += 0 * fake_gt_img[0, 0, 0, 0] * fake_gt_img_bin[0, 0, 0, 0]
