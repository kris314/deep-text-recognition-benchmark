        ...
        if not opt.testFlag:
            ...
            if opt.zAlone:
                ...
                if opt.reconWeight>0.0:
                    imgReconCost = reconCriterion(fake_img, image_output_tensors)
                else:
                    imgReconCost = torch.tensor(0.0)
                ...
                if opt.vggPerWeight>0.0 or opt.vggStyWeight>0.0 or opt.vggEmbWeight>0.0:
                    vggPerCost , vggStyCost, vggEmbCost  = vggModel(fake_img, image_output_tensors, inAct=opt.taskActivation, normFlag=not(opt.vggNoMean))
                else:
                    vggPerCost = torch.tensor(0.0)
                    vggStyCost = torch.tensor(0.0)
                    vggEmbCost = torch.tensor(0.0)
