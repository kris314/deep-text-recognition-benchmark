...
def train(opt):
    ...
    if not opt.zAlone:
        ...
        styleModel = ResNet_FeatureExtractor(opt.input_channel, opt.latent)
        ...
        vggModel = VGGPerceptualLossModel(models.vgg19(pretrained=True), reconCriterion)
    else:
        c_code_size = 0

    
    genModel = styleGANGen(opt.size, opt.latent, opt.latent, opt.n_mlp, content_dim=c_code_size, channel_multiplier=opt.channel_multiplier)
    g_ema = styleGANGen(opt.size, opt.latent, opt.latent, opt.n_mlp, content_dim=c_code_size, channel_multiplier=opt.channel_multiplier)
    disEncModel = styleGANDis(opt.size, channel_multiplier=opt.channel_multiplier, input_dim=opt.input_channel, code_s_dim=opt.latent)
    ...
    if opt.saved_gen_model !='' and opt.saved_gen_model !='None':
        ...
        genModel.module.load_state_dict(checkpoint['genModel'])
    ...
    while(True):
        ...
        if opt.zAlone:
            ...
            if opt.reconWeight>0.0 or opt.vggStyWeight>0.0:
            ...
            if opt.vggStyWeight>0.0:
                ...
                _ , vggStyleCost = vggModel(fake_img, gt_image_tensors)
                vggPerCost = torch.tensor(0.0)
            else:
                vggPerCost = torch.tensor(0.0)
                vggStyCost = torch.tensor(0.0)
        ...
        if opt.grad_balance and not opt.zAlone:
            ...
            grad_fake_OCR = torch.autograd.grad(ocrCost, fake_img, create_graph=True, retain_graph=True)[0]
            grad_fake_adv = torch.autograd.grad(disGenCost, fake_img, create_graph=True, retain_graph=True)[0]
            grad_fake_sty_vgg = torch.autograd.grad(vggStyleCost, fake_img, create_graph=True, retain_graph=True)[0]
        ...
        loss_avg_style_code.add(opt.gamma_g * uCost)
        ...
        if not opt.zAlone:
            if (iteration + 1) % opt.valInterval == 0 or iteration == 0:
                ...
                preds = ocrModel(fake_gt_img, text_gt, is_train=False)
                ...
                for trImgCntr in range(opt.batch_size):
                    try:
                        save_image(tensor2im(fake_img[trImgCntr].detach()),os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_z_'+labels_z_c[trImgCntr]+'_ocr:'+preds_str_fake_img[trImgCntr]+'.png'))
                        save_image(tensor2im(fake_gt_img[trImgCntr].detach()),os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_gt_'+labels_gt[trImgCntr]+'_ocr:'+preds_str_fake_gt_img[trImgCntr]+'.png'))
                    except:
        ...
        if (iteration + 1) % opt.valInterval == 0 or iteration == 0: # To see training progress, we also conduct validation when 'iteration == 0' 
            #validation
            image_input_tensors, _, labels, _ = iter(valid_loader).next()
            ...
            with torch.no_grad():
                ...
                if not opt.zAlone:
                    ...
                    if 'CTC' in opt.Prediction:
                        ...
                        preds = ocrModel(gt_image_tensors_1, text_gt_2, is_train=False)
                        preds_size = torch.IntTensor([preds.size(1)] * gt_image_tensors.shape[0])
                        _, preds_index = preds.max(2)
            ...
            with open(os.path.join(opt.exp_dir,opt.exp_name,'log_train.txt'), 'a') as log:
                ...
                loss_log = f'[{iteration+1}/{opt.num_iter}]  \
                    ...
                    Train StyleCode loss: {loss_avg_style_code.val():0.5f}, Train Image Recon loss: {loss_avg_img_recon.val():0.5f}, \
                ...
                lib.plot.plot(os.path.join(opt.plotDir,'Train-StyleCode-Loss'), loss_avg_style_code.val().item())
