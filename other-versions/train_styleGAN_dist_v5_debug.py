...
from model_word import GeneratorM2V4 as styleGANGen 
...
def train(opt):
    ...
    while(True):
        ...
        if opt.zAlone:
            ...
            if opt.reconWeight>0.0:
                imgReconCost = reconCriterion(fake_gt_img, gt_image_tensors)
            else:
                imgReconCost = torch.tensor(0.0)
            
            if opt.vggStyWeight>0.0:
                _ , vggStyCost = vggModel(fake_img, gt_image_tensors, inAct=opt.taskActivation)
                vggPerCost = torch.tensor(0.0)
            else:
                vggPerCost = torch.tensor(0.0)
                vggStyCost = torch.tensor(0.0)
        ...
        if opt.grad_balance and not opt.zAlone:
            ...
            if b is None:
                print(ocrCost, disGenCost, torch.std(grad_fake_adv), torch.std(grad_fake_sty_vgg))
        ...
        if not opt.zAlone:
            if get_rank() == 0 and ((iteration + 1) % opt.valInterval == 0 or iteration == 0):
                ...
                for trImgCntr in range(opt.batch_size):
                    try:
                        if not opt.zAlone:
                            utils.save_image(gt_image_tensors[trImgCntr],os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_tr_gt_orig_'+labels_gt[trImgCntr]+'_ocr:None.png'),nrow=1,normalize=True,range=(-1, 1))
                        ...
                        if opt.cEncode == 'cnn':
                            utils.save_image(synth_z_c[trImgCntr],os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_tr_synth_'+labels_z_c[trImgCntr]+'_ocr:None.png'),nrow=1,normalize=True,range=(-1, 1))
                        ...
                        utils.save_image(fake_img[trImgCntr],os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_tr_z_'+labels_z_c[trImgCntr]+'_ocr:'+preds_str_fake_img[trImgCntr]+'_sty:'+labels_gt[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))
                        ...
                        if opt.reconWeight>0.0:
                            utils.save_image(fake_gt_img[trImgCntr],os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_tr_gt_pred_'+labels_gt[trImgCntr]+'_ocr:'+preds_str_fake_gt_img[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))
        ...
        if get_rank() == 0:
            ...
            if (iteration + 1) % opt.valInterval == 0 or iteration == 0: # To see training progress, we also conduct validation when 'iteration == 0' 
                ...
                with torch.no_grad():
                    ...
                    fake_img_c1_s1, _ = g_ema(style_1, z_c_code_1, input_is_latent=opt.input_latent)
                    if not opt.zAlone:
                        fake_img_c2_s1, _ = g_ema(style_1, z_c_code_2, input_is_latent=opt.input_latent)
                        fake_img_c1_s2, _ = g_ema(style_2, z_c_code_1, input_is_latent=opt.input_latent)
                ...
                for trImgCntr in range(opt.batch_size):
                    try:
                        ...
                        utils.save_image(fake_img_c1_s1[trImgCntr],os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_val_c1_s1_'+labels_z_c_1[trImgCntr]+'_ocr:'+preds_str_fake_img_c1_s1[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))
                        if not opt.zAlone:
                            utils.save_image(gt_image_tensors_1[trImgCntr],os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_val_s1_'+labels_gt_1[trImgCntr]+'_ocr:'+preds_str_gt_1[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))
                            utils.save_image(gt_image_tensors_2[trImgCntr],os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_val_s2_'+labels_gt_2[trImgCntr]+'_ocr:'+preds_str_gt_2[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))

                            # save_image(tensor2im(gt_image_tensors_1[trImgCntr].detach()),os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_val_s1_'+labels_gt_1[trImgCntr]+'_ocr:'+preds_str_gt_1[trImgCntr]+'.png'))
                            # save_image(tensor2im(gt_image_tensors_2[trImgCntr].detach()),os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_val_s2_'+labels_gt_2[trImgCntr]+'_ocr:'+preds_str_gt_2[trImgCntr]+'.png'))
                            
                            utils.save_image(fake_img_c2_s1[trImgCntr],os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_val_c2_s1_'+labels_z_c_2[trImgCntr]+'_ocr:'+preds_str_fake_img_c2_s1[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))
                            utils.save_image(fake_img_c1_s2[trImgCntr],os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_val_c1_s2_'+labels_z_c_1[trImgCntr]+'_ocr:'+preds_str_fake_img_c1_s2[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))

                            # save_image(tensor2im(fake_img_c2_s1[trImgCntr].detach()),os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_val_c2_s1_'+labels_z_c_2[trImgCntr]+'_ocr:'+preds_str_fake_img_c2_s1[trImgCntr]+'.png'))
                            # save_image(tensor2im(fake_img_c1_s2[trImgCntr].detach()),os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_val_c1_s2_'+labels_z_c_1[trImgCntr]+'_ocr:'+preds_str_fake_img_c1_s2[trImgCntr]+'.png'))

                            if opt.cEncode == 'cnn':
                                utils.save_image(synth_z_c_1[trImgCntr],os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_val_c1_synth_'+labels_z_c_1[trImgCntr]+'_ocr:None.png'),nrow=1,normalize=True,range=(-1, 1))
                                utils.save_image(synth_z_c_2[trImgCntr],os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_val_c2_synth_'+labels_z_c_2[trImgCntr]+'_ocr:None.png'),nrow=1,normalize=True,range=(-1, 1))

                                # save_image(tensor2im(synth_z_c_1[trImgCntr].detach()),os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_val_c1_synth_'+labels_z_c_1[trImgCntr]+'_ocr:None.png'))
                                # save_image(tensor2im(synth_z_c_2[trImgCntr].detach()),os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_val_c2_synth_'+labels_z_c_2[trImgCntr]+'_ocr:None.png'))
                            # if trImgCntr<gt_image_tensors.shape[0]:
                            #     save_image(tensor2im(gt_image_tensors[trImgCntr].detach()),os.path.join(opt.valDir,str(iteration),str(trImgCntr)+'_gt_act:'+labels_gt[trImgCntr]+'_ocr:'+preds_str_gt[trImgCntr]+'.png'))
                    except:
