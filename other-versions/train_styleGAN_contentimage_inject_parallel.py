...
from dataset import hierarchical_dataset, AlignPairImgCollate, Batch_Balanced_Dataset, tensor2im, save_image
...
from model_word import ConditionalImageMGPUGenerator as styleGANGen 
...
def train(opt):
    ...
    genModel = styleGANGen(opt.size, opt.latent, opt.n_mlp, opt.num_class, channel_multiplier=opt.channel_multiplier, content_inject_index=opt.content_inject_index)
    disModel = styleGANDis(opt.size, channel_multiplier=opt.channel_multiplier, input_dim=opt.input_channel)
    g_ema = styleGANGen(opt.size, opt.latent, opt.n_mlp, opt.num_class, channel_multiplier=opt.channel_multiplier, content_inject_index=opt.content_inject_index)
    ...
    while(True):
        ...
        image_input_tensors, image_gt_tensors, labels_1, labels_2, synthimg_labels_2_tensors = iter(train_loader).next()
        ...
        images_recon_2, _ = genModel(style, synthimg_labels_2_tensors)
        ...
        if g_regularize:
            ...
            images_recon_2, grad = genModel(style, synthimg_labels_2_tensors[:path_batch_size], return_latents=True, g_path_regularize=True, mean_path_length=mean_path_length)
        ...
        if (iteration + 1) % opt.valInterval == 0 or iteration == 0: # To see training progress, we also conduct validation when 'iteration == 0' 
            ...
            images_recon_2, _ = g_ema(style, synthimg_labels_2_tensors[:curr_batch_size], input_is_latent=opt.input_latent)
            ...
            with open(os.path.join(opt.exp_dir,opt.exp_name,'log_train.txt'), 'a') as log:
                ...
                with torch.no_grad():                    
                    valid_loss, infer_time, length_of_data = validation_synth_v7(
...
if __name__ == '__main__':
    ...
    parser.add_argument("--style_content_input", action="store_true")
