...
def train(opt):
    ...
    optimizer = optim.Adam(
        list(styleModel.parameters())+ list(mixModel.parameters())+ list(genModel.parameters()),
    ...
    while(True):
        ...
        # images_recon_2,_ = genModel([scInput[:,:opt.latent],scInput[:,opt.latent:]])
        noise = mixing_noise(batch_size, opt.latent, opt.mixing, device)
        
        images_recon_2, _ = genModel(noise)
        ...
        if g_regularize:
            ...
            # images_recon_2, latents = genModel([scInput[:,:opt.latent],scInput[:,opt.latent:]], return_latents=True)
            noise = mixing_noise(path_batch_size, opt.latent, opt.mixing, device)
            images_recon_2, latents = genModel(noise, return_latents=True)
        ...
        if (iteration + 1) % opt.valInterval == 0 or iteration == 0: # To see training progress, we also conduct validation when 'iteration == 0' 
            ...
            with open(os.path.join(opt.exp_dir,opt.exp_name,'log_train.txt'), 'a') as log:
                ...
                #         iteration, styleModel, g_ema, mixModel, vggModel, ocrModel, disModel, recCriterion, ocrCriterion, valid_loader, converter, opt)
