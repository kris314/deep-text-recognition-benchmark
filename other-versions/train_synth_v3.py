...
def train(opt):
    ...
    while(True):
        ...
        real_pred = disModel(torch.cat((image_gt_tensors_aug,image_input_tensors),dim=1))
        ...
        if d_regularize:
            ...
            cat_tensor = torch.cat((image_gt_tensors_aug,image_input_tensors),dim=1)
        ...
        if (iteration + 1) % opt.valInterval == 0 or iteration == 0: # To see training progress, we also conduct validation when 'iteration == 0' 
            ...
            # pdb.set_trace()
            images_recon_2, _ = g_ema([scInput], input_is_latent=opt.input_latent)
