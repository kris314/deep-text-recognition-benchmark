...
from model_word import Conditional16Generator as styleGANGen 
...
def train(opt):
    ...
    genModel = styleGANGen(opt.size, opt.latent, opt.n_mlp, opt.num_class, channel_multiplier=opt.channel_multiplier)
    ...
    while(True):
        ...
        style = mixing_noise_style(opt.batch_size, opt.latent, opt.mixing, device)
        # style=[]
        ...
        fake_img,_ = genModel(style, text_z_c, input_is_latent=opt.input_latent)
        ...
        if g_regularize:
            ...
            # z_c_code = cEncoder(text_z_c)
            style = mixing_noise_style(path_batch_size, opt.latent, opt.mixing, device)
            # style=[]
            # style.append(noise_style[0]*z_c_code)
            # if len(noise_style)>1:
            #     style.append(noise_style[1]*z_c_code)
            ...
            fake_img, grad = genModel(style, text_z_c, return_latents=True, g_path_regularize=True, mean_path_length=mean_path_length)
        ...
        if (iteration + 1) % opt.valInterval == 0 or iteration == 0: # To see training progress, we also conduct validation when 'iteration == 0' 
            ...
            style_s2 = mixing_noise_style(opt.batch_size, opt.latent, opt.mixing, device)
            ...
            fake_img_c1_s1, _ = g_ema(style_s1, text_z_c_1, input_is_latent=opt.input_latent)
            fake_img_c2_s1, _ = g_ema(style_s1, text_z_c_2, input_is_latent=opt.input_latent)
            fake_img_c1_s2, _ = g_ema(style_s2, text_z_c_1, input_is_latent=opt.input_latent)
