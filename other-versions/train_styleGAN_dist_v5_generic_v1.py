            ...
            if not opt.zAlone:
                if get_rank() == 0 and ((iteration + 1) % opt.valInterval == 0 or iteration == 0):
                    ...
                    for trImgCntr in range(opt.batch_size):
                        try:
                            if not opt.zAlone:
                                utils.save_image(image_output_tensors[trImgCntr],os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_gt_orig_'+labels_gt[trImgCntr]+'_ocr:None'+'.png'),nrow=1,normalize=True,range=(-1, 1))
                            if opt.cEncode == 'cnn':
