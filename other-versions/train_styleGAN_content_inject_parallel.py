...
from model_word import ConditionalCharEmbedMGPUGenerator as styleGANGen 
...
def train(opt):
    ...
    while(True):
        ...
        if g_regularize:
            ...
            if 'CTC' in opt.Prediction:
                images_recon_2, grad = genModel(style, text_2[:path_batch_size], return_latents=True, g_path_regularize=True, mean_path_length=mean_path_length)
            else:
                images_recon_2, grad = genModel(style, text_2[:path_batch_size,1:-1], return_latents=True, g_path_regularize=True, mean_path_length=mean_path_length)
