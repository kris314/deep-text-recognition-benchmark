...
def train(opt):
    ...
    while(True):
        ...
        if opt.augment:
            image_input_tensors_aug, _ = augment(image_input_tensors, ada_aug_p)
            images_recon_2, _ = augment(images_recon_2, ada_aug_p)

        else:
            image_input_tensors_aug = image_input_tensors

        fake_pred = disModel(images_recon_2)
        real_pred = disModel(image_input_tensors_aug)
        ...
        if d_regularize:
            ...
            image_input_tensors.requires_grad = True
            cat_tensor = image_input_tensors
        ...
        # validation part
        if ((iteration + 1) % opt.valInterval == 0 or iteration == 0) and get_rank() == 0 : # To see training progress, we also conduct validation when 'iteration == 0' 
            g_ema.eval()
            disModel.eval()
            ...
            fake_pred = disModel(images_recon_2)
            real_pred = disModel(image_input_tensors)
            disCost = d_logistic_loss(real_pred, fake_pred)
            disGenCost = g_nonsaturating_loss(fake_pred)
        ...
        if (iteration) % 3000 == 0 and get_rank() == 0:
            torch.save({
