...
from model_word import PixelNorm, StyleTensorEncoderV6, ContentTensorEncoderV6
...
def train(opt):
    ...
    if not opt.zAlone:
        ...
        if opt.cEncode == 'mlp':
            ...
            cEncoder = ContentTensorEncoderV6(n_downsample=4, n_res=0, input_dim=1, dim=16, norm=opt.contentNorm, activ='lrelu').to(device)
            sEncoder = ContentTensorEncoderV6(n_downsample=4, n_res=2, input_dim=3, dim=16, norm=opt.styleNorm, activ='lrelu').to(device)
            c_code_size = 0 #for 5-1/v6 architecture
        # styleModel = ResNet_StyleExtractor_WBN(opt.input_channel, opt.latent).to(device)
        styleModel = StyleTensorEncoderV6(n_downsample=4, n_res=4, input_dim=opt.input_channel, dim=32, norm=opt.styleNorm, activ='lrelu', numLatents=opt.numLatents).to(device)
    ...
    if not opt.zAlone and opt.distributed:
        ...
        sEncoder = torch.nn.parallel.DistributedDataParallel(
            sEncoder,
            device_ids=[opt.local_rank],
            output_device=opt.local_rank,
            broadcast_buffers=False,
    ...
    if opt.distributed:
        if not opt.zAlone:
            ...
            sEncoder_module = sEncoder.module
    ...
    if opt.zAlone:
        ...
        optimizer = optim.Adam(
            list(genModel.parameters())+list(cEncoder.parameters())+list(styleModel.parameters())+list(sEncoder.parameters()),
            lr=opt.lr * g_reg_ratio,
            betas=(0 ** g_reg_ratio, 0.99 ** g_reg_ratio),
        )
        if opt.optim == "adam":
    ...
    if opt.saved_synth_model != '' and opt.saved_synth_model != 'None':
        ...
        if not opt.zAlone:
            ...
            sEncoder_module.load_state_dict(checkpoint['sEncoder'])
    ...
    imgStyleReconCost = torch.tensor(0.0, device=device)
    # loss_dict = {}
        ...
        if not opt.testFlag:
            ...
            if opt.zAlone:
                ...
                if opt.cEncode == 'mlp':    
                    ...
                    z_s_code = sEncoder(gt_image_tensors)
            ...
            fake_img,_ = genModel(style, torch.cat((z_c_code,z_s_code),dim=1), input_is_latent=opt.input_latent, taskActivation=opt.taskActivation)
            ...
            if opt.zAlone:
                ...
                if opt.reconWeight>0.0:
                    ...
                    fake_img_fake ,_ = genModel(style_fake, torch.cat((z_gt_code,z_s_fake),dim=1), input_is_latent=opt.input_latent, taskActivation=opt.taskActivation)
            ...
            # Word generator update 2
            g_imgRecon_flag = (cntr % opt.g_imgStyle_every == 0) and (opt.imgStyleReconWeight!=0)

            if g_imgRecon_flag:
                image_input_tensors, _, labels, _, labelSynthImg,_  = next(train_loader)
                labels_z_c, synth_z_c = labels, labelSynthImg
                ...
                imgStyleReconCost = reconCriterion(fake_img, gt_image_tensors) * opt.imgStyleReconWeight * opt.g_imgStyle_every

                imgStyleReconCost.backward()
                optimizer.step()

            log_imgStyle_recon.add(imgStyleReconCost)
            ...
            if g_regularize:
                ...
                fake_img, latents = genModel(style, torch.cat((z_c_code,z_s_code),dim=1), return_latents=True, input_is_latent=opt.input_latent, taskActivation=opt.taskActivation)
            ...
            if (iteration + 1) % opt.valInterval == 0 or iteration == 0 or opt.testFlag: # To see training progress, we also conduct validation when 'iteration == 0' 
                ...
                with torch.no_grad():
                    if opt.zAlone:
                        ...
                        if opt.cEncode == 'mlp':    
                            ...
                            z_s_code_1 = sEncoder(gt_image_tensors_1)
                            z_s_code_2 = sEncoder(gt_image_tensors_2)
                    ...
                    fake_img_c1_s1, _ = g_ema(style_1, torch.cat((z_c_code_1, z_s_code_1),dim=1), input_is_latent=opt.input_latent, taskActivation=opt.taskActivation)
                    if not opt.zAlone:
                        fake_img_c2_s1, _ = g_ema(style_1, torch.cat((z_c_code_2, z_s_code_1),dim=1), input_is_latent=opt.input_latent, taskActivation=opt.taskActivation)
                        fake_img_c1_s2, _ = g_ema(style_2, torch.cat((z_c_code_1, z_s_code_2),dim=1), input_is_latent=opt.input_latent, taskActivation=opt.taskActivation)
                ...
                for trImgCntr in range(opt.batch_size):
                    ...
                    try:
                        ...
                        content_c1_s1 = 'PSTYLE-1 '+'Text-1:' + labels_z_c_1[trImgCntr]+' OCR:' + preds_str_fake_img_c1_s1[trImgCntr]
                        content_s1 = 'OSTYLE-1 '+'GT:' + labels_gt_1[trImgCntr]+' OCR:' + preds_str_gt_1[trImgCntr]
                        content_s2 = 'OSTYLE-2 '+'GT:' + labels_gt_2[trImgCntr]+' OCR:'+preds_str_gt_2[trImgCntr]
                        content_c2_s1 = 'PSTYLE-1 '+'Text-2:' + labels_z_c_2[trImgCntr]+' OCR:'+preds_str_fake_img_c2_s1[trImgCntr]
                        content_c1_s2 = 'PSTYLE-2 '+'Text-1:' + labels_z_c_1[trImgCntr]+' OCR:'+preds_str_fake_img_c1_s2[trImgCntr]
                ...
                if not opt.testFlag:
                    ...
                    with open(os.path.join(opt.exp_dir,opt.exp_name,'log_train.txt'), 'a') as log:
                        ...
                        loss_log = f'[{iteration+1}/{opt.num_iter}]  \
                            ...
                            Train Dis StyleCode loss: {loss_avg_style_ucode.val():0.5f}, Train Gen StyleCode loss: {loss_avg_style_scode.val():0.5f}, \
                            Train Cycle Image Recon loss: {loss_avg_img_recon.val():0.5f}, Train Style Image Recon loss: {log_imgStyle_recon.val():0.5f}, \
                        ...
                        lib.plot.plot(os.path.join(opt.plotDir,'Train-CycleImageRecon-Loss'), loss_avg_img_recon.val().item())
                        lib.plot.plot(os.path.join(opt.plotDir,'Train-StyleImageRecon-Loss'), log_imgStyle_recon.val().item())
            ...
            if not opt.testFlag:
                ...
                if (iteration) % 3000 == 0:
                    if opt.zAlone:
                        ...
                        'sEncoder':sEncoder_module.state_dict(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--numLatents', type=int, default=1, help='1/10')
    parser.add_argument("--g_imgStyle_every", type=int, default=4)
    parser.add_argument('--imgStyleReconWeight', type=float, default=1.0, help='weights for img style recon')
