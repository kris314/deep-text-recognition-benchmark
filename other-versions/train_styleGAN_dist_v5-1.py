...
from model import ModelV1, GlobalContentEncoder, VGGPerceptualLossModel, StyleTensorEncoder, Fusion
...
from model_word import PixelNorm
...
def train(opt):
    ...
    if not opt.zAlone:
        ...
        if opt.cEncode == 'mlp':
            ...
            cEncoder = StyleTensorEncoder(n_downsample=3, n_res=1, input_dim=1, dim=32, norm=opt.styleNorm, activ='lrelu').to(device)
            c_code_size = 0 #for 5-1 architecture
        # styleModel = ResNet_StyleExtractor_WBN(opt.input_channel, opt.latent).to(device)
        styleModel = StyleTensorEncoder(n_downsample=3, n_res=4, input_dim=opt.input_channel, dim=64, norm=opt.styleNorm, activ='lrelu').to(device)
    ...
    if not opt.zAlone:
        for model in [styleModel, cEncoder, fusionModel]:
    ...
    if opt.zAlone:
        ...
        optimizer = optim.Adam(
            list(genModel.parameters())+list(cEncoder.parameters())+list(styleModel.parameters())+list(fusionModel.parameters()),
            lr=opt.lr * g_reg_ratio,
            betas=(0 ** g_reg_ratio, 0.99 ** g_reg_ratio),
        )
        if opt.optim == "adam":
    ...
    while(True):
        ...
        if not opt.testFlag:
            ...
            fake_img,_ = genModel(style, None, input_is_latent=opt.input_latent, taskActivation=opt.taskActivation)
            ...
            if opt.zAlone:
                ...
                if opt.reconWeight>0.0:
                    ...
                    fake_img_fake ,_ = genModel(style_fake, None, input_is_latent=opt.input_latent, taskActivation=opt.taskActivation)
            ...
            if g_regularize:
                ...
                fake_img, latents = genModel(style, None, return_latents=True, input_is_latent=opt.input_latent, taskActivation=opt.taskActivation)
                ...
                with torch.no_grad():
                    if opt.zAlone:
                        ...
                        if opt.normFeat:
                            ...
                            style_1_cont_1 = style_1_cont_1 * torch.rsqrt(torch.mean(style_1_cont_1 ** 2, dim=1, keepdim=True) + 1e-8)
                            style_1_cont_2 = style_1_cont_2 * torch.rsqrt(torch.mean(style_1_cont_2 ** 2, dim=1, keepdim=True) + 1e-8)
                            style_2_cont_1 = style_2_cont_1 * torch.rsqrt(torch.mean(style_2_cont_1 ** 2, dim=1, keepdim=True) + 1e-8)
                            
                            
                    if opt.noiseConcat or opt.zAlone:
                        style_1_cont_1 = mixing_noise(opt.batch_size, opt.latent, opt.mixing, device, style_1_cont_1)
                        style_1_cont_2 = mixing_noise(opt.batch_size, opt.latent, opt.mixing, device, style_1_cont_2)
                        style_2_cont_1 = mixing_noise(opt.batch_size, opt.latent, opt.mixing, device, style_2_cont_1)
                    else:
                        style_1_cont_1 = [style_1_cont_1]
                        style_1_cont_2 = [style_1_cont_2]
                        style_2_cont_1 = [style_2_cont_1]
                                
                    fake_img_c1_s1, _ = g_ema(style_1_cont_1, None, input_is_latent=opt.input_latent, taskActivation=opt.taskActivation)
                    if not opt.zAlone:
                        fake_img_c2_s1, _ = g_ema(style_1_cont_2, None, input_is_latent=opt.input_latent, taskActivation=opt.taskActivation)
                        fake_img_c1_s2, _ = g_ema(style_2_cont_1, None, input_is_latent=opt.input_latent, taskActivation=opt.taskActivation)
