    ...
    while(True):
        ...
        if not opt.testFlag:
            ...
            if not opt.ocrFixed and not opt.zAlone:
                ...
                if 'CTC' in opt.Prediction:
                    preds_recon = ocrModel(gt_image_tensors, text_gt, is_train=True, inAct = opt.taskActivation)
                    ...
                    preds_recon = ocrModel(gt_image_tensors, text_gt[:, :-1], is_train=True, inAct = opt.taskActivation)  # align with Attention.forward
                ...
                if get_rank() == 0 and ((iteration + 1) % opt.valInterval == 0 or iteration == 0):
                    ...
                    for trImgCntr in range(opt.batch_size):
                        try:
                            ...
                            if opt.reconWeight>0.0:
                                utils.save_image(fake_gt_img[trImgCntr],os.path.join(opt.trainDir,str(iteration),str(trImgCntr)+'_tr_gt_pred_'+labels_gt[trImgCntr]+'_ocr:'+preds_str_fake_gt_img[trImgCntr]+'.png'),nrow=1,normalize=True,range=(-1, 1))
                ...
                with torch.no_grad():
                    ...
                    if not opt.zAlone:
                        ...
                        if 'CTC' in opt.Prediction:
                            ...
                            preds = ocrModel(fake_img_c1_s1, text_z_c_1[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                            _, preds_index = preds.max(2)
                            ...
                            preds = ocrModel(fake_img_c2_s1, text_z_c_2[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                            _, preds_index = preds.max(2)
                            ...
                            preds = ocrModel(fake_img_c1_s2, text_z_c_1[:, :-1], is_train=False, inAct = opt.taskActivation)  # align with Attention.forward
                            _, preds_index = preds.max(2)
                            preds_str_fake_img_c1_s2 = converter.decode(preds_index, length_for_pred)
                            for idx, pred in enumerate(preds_str_fake_img_c1_s2):
                                pred_EOS = pred.find('[s]')
                                preds_str_fake_img_c1_s2[idx] = pred[:pred_EOS]  # prune after "end of sentence" token ([s])

                            preds = ocrModel(gt_image_tensors_1, text_gt_1[:, :-1], is_train=False)  # align with Attention.forward
                            _, preds_index = preds.max(2)
                            ...
                            preds = ocrModel(gt_image_tensors_2, text_gt_2[:, :-1], is_train=False)  # align with Attention.forward
                            _, preds_index = preds.max(2)
